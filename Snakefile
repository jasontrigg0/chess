...
#step 1:
#find f ("loss score") such that f(position,move) correlates best with elo
#eg: elo ~ loss, loss * eval**2, loss * loss_8, loss_sq, loss * (move_cnt < 10)

#less /tmp/chess_features.csv | pcsv -p 'r["abs_eval"] = abs(float(r["eval"])); r["negative_loss"] = 1*(float(r["loss"])<0); r["best_move"] = 1* ( r["loss"] == "0" ); r["loss_X_abs_eval"] = float(r["abs_eval"]) * float(r["loss"]); r["loss_X_eval_sq"] = (float(r["abs_eval"])**2) * float(r["loss"]); r["l2"] = float(r["loss"]) * (float(r["abs_eval"]) == 1000); r["loss_X_loss8"] = float(r["loss"]) * float(r["loss_8"]); r["move_cnt"] = float(r["loss"]) * (float(r["position"].split()[-1]) < 10); r["loss_sq"] = float(r["loss"])**2; r["abs_eval_X_loss"] = float(r["abs_eval"]) * float(r["loss"]); r["abs_eval_X_loss_sq"] = float(r["abs_eval"]) * float(r["loss_sq"]); r["loss_X_max_eval"] = float(r["loss"]) * (float(r["abs_eval"]) == 1000)' | linreg -c loss,loss_X_loss8,move_cnt,loss_sq,abs_eval_X_loss,abs_eval_X_loss_sq -t elo  --so > /tmp/preds1.csv

#step 2:
#best prediction g ("expected_loss_score") for g(position,elo) = f(position,move)
#eg: f(loss) ~ abs(eval), elo, loss_8

less /tmp/preds1.csv | pagg -c elo -a mean --append | pcsv -p 'r["loss_score"] = float(r["pred"]); r["max_eval"] = 1 * (float(r["abs_eval"]) == 1000)' | pcsv -C pred,error | linreg -c abs_eval,max_eval,elo,loss_0,loss_1,loss_2,loss_4,loss_8 -t loss_score  --so > /tmp/preds2.csv
...
#COEFF=0.00355759831763561; less /tmp/preds2.csv | pcsv -p 'r["expected_loss_score"] = float(r["pred"]) - '"$COEFF"' * (float(r["elo"]) - float(r["elo_mean"])); r["adjusted_loss_score"] = float(r["loss_score"]) - float(r["expected_loss_score"])' | pcsv -C pred,error  | pcsv -p 'r["als_var"] = float(r["adjusted_loss_score"])**2' | linreg -c abs_eval -t als_var --so | pcsv -p 'r["weight"] = 1 / float(r["pred"])' | pcsv -C pred,error > /tmp/preds3.csv
